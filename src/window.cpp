    ...
    watcher(new QFileSystemWatcher(this))
{
    ...
    // Surface Format for Qt 6
    QSurfaceFormat format;
    format.setDepthBufferSize(24);
    format.setStencilBufferSize(8);
    format.setVersion(4,1);
    format.setProfile(QSurfaceFormat::CompatibilityProfile);   // Change to CompatibilityProfile if wireframe rendering fails
...
void Window::load_persist_settings()
{
    ...
    DrawMode draw_mode = static_cast<DrawMode>(settings.value(DRAW_MODE_KEY, DRAWMODECOUNT).toInt());
...
void Window::on_open_external()
{
    ...
    if (program.isEmpty()) {
        program = QFileDialog::getOpenFileName(
            this, "Select program to open with", QDir::rootPath());
...
void Window::on_about()
{
    QMessageBox::about(this, "",
                       ...
                       "<a href=\"https://github.com/fstl-app/fstl\" style=\"color: #93a1a1;\">"
                       "https://github.com/fstl-app/fstl</a></p>"
                       "<p>© 2014-2025 Matthew Keeter<br>"
                       "<a href=\"mailto:matt.j.keeter@gmail.com\" style=\"color: #93a1a1;\">"
                       "matt.j.keeter@gmail.com</a></p>");
}
...
void Window::on_drawMode(QAction* act)
{
    ...
    QSettings().setValue(DRAW_MODE_KEY, static_cast<int>(mode));
}
...
void Window::on_save_screenshot()
{
    ...
    if (!image.save(file_name)) {
        QMessageBox::warning(this, tr("Error Saving Image"),
                             tr("Unable to save screenshot image."));
    }
}
...
void Window::rebuild_recent_files()
{
    ...
    for (const auto& f : files) {
        auto a = new QAction(f, recent_files);
        a->setData(f);
...
void Window::on_common_view_change(QAction* common)
{
    ...
    else if (common == common_view_iso_action) canvas->common_view_change(isoview);
    else if (common == common_view_top_action) canvas->common_view_change(topview);
    else if (common == common_view_bottom_action) canvas->common_view_change(bottomview);
    else if (common == common_view_left_action) canvas->common_view_change(leftview);
    else if (common == common_view_right_action) canvas->common_view_change(rightview);
    else if (common == common_view_front_action) canvas->common_view_change(frontview);
    else if (common == common_view_back_action) canvas->common_view_change(backview);
}
...
void Window::build_folder_file_list()
{
    ...
    if (!lookup_folder_files.isEmpty() && current_folder_path == lookup_folder) {
        return;
    }
    ...
    while (dirIterator.hasNext()) {
        dirIterator.next();
        sorted_insert(lookup_folder_files, collator, dirIterator.fileName());
    }
}
